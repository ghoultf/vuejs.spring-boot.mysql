package com.taskagile.app.infrastructure.file.s3;
...
import com.taskagile.app.domain.common.file.AbstractBaseFileStorage;
import com.taskagile.app.domain.common.file.FileStorageException;
import com.taskagile.app.domain.common.file.TempFile;
...
public class S3FileStorage extends AbstractBaseFileStorage {
  ...
  public S3FileStorage(Environment environment, @Value("${app.file-storage.temp-folder}") String rootTempPath) {
  ...
  public String saveUploaded(String folder, MultipartFile multipartFile) {
    ...
    try {
      ...
      PutObjectRequest putRequest = new PutObjectRequest(bucketName, s3ObjectKey, multipartFile.getInputStream(),
          metadata);
      putRequest.withCannedAcl(CannedAccessControlList.PublicRead);
      s3.putObject(putRequest);
  ...
  private AmazonS3 initS3Client() {
    ...
    if (environment.acceptsProfiles("dev")) {
      ...
      return AmazonS3ClientBuilder.standard().withCredentials(new InstanceProfileCredentialsProvider(false))
