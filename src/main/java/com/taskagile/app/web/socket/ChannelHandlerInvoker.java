...
public class ChannelHandlerInvoker {
  ...
  public void handle(IncomingMessage incomingMessage, RealTimeSession session) {
    Assert.isTrue(antPathMatcher.match(channelPattern, incomingMessage.getChannel()),
        "Channel of the handler must match");
    ...
    try {
        ...
        if (parameterAnnotation instanceof Payload) {
          ...
          if (arg == null) {
            throw new IllegalArgumentException(
                "Unable to instantiate parameter of type `" + parameterType.getName() + "`.");
          }
          args[i] = arg;
      ...
      String error = "Failed to invoker action method `" + incomingMessage.getAction() + "` at channel `"
